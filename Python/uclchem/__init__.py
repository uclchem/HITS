...
def phase_one(param_dict):
	...
	abunds=wrap.phaseone(dictionary=param_dict, outspeciesin=outSpecies)
	return 0

def phase_two(param_dict):
	outSpecies = (param_dict['outSpecies'])
	param_dict['outSpecies'] = len(outSpecies.split())
	history=param_dict.pop("history")
	if history=="No History":
		wrap.run_model_to_file(dictionary=param_dict, outspeciesin=outSpecies)
	else:
		abundances=pd.read_csv(history,skiprows=2)
		abundances=abundances.iloc[-1].values
		abundances=abundances[5:]
		wrap.phasetwo(dictionary=param_dict, outspeciesin=outSpecies,abundancesin=abundances)
	return 0
